...
export const MyTable: FC<{ rows: Data[] }> = ({ rows }) => {
    ...
    function sort_filter(array: Data[], comparator: (a: any, b: any) => number): Data[] {
        ...
        const filtered = search.length > 0
            ? new_arr.filter(([el]) => el.value.indexOf(search) > -1) // filter on name or email
            : new_arr;
                                ...
                                .map((row, index) => {
                                    ...
                                    return (
                                        ...
                                        >
                                            ...
                                            <TableCell >{row.docs && Array.isArray(row.docs) ? row.docs.join('  ') : row.docs}</TableCell>
                                            <TableCell >{row.sentences && Array.isArray(row.sentences) ? row.sentences.join('  ') : row.sentences}</TableCell>
                                        </TableRow>
                                    );
                                })}
                        </TableBody>
